...
public class AnalyticsServiceHttpAdapter implements AnalyticsServicePort {
    ...
    public CompletableFuture<Void> ingestEvent(JsonObject event) {
        ...
        return httpClient.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                .thenApply(response -> {
                    logger.info("✅ [AnalyticsService] ingest event response received - Status: {}", response.statusCode());
                    if (response.statusCode() >= 200 && response.statusCode() < 300) {
                        logger.debug("   Response body: {}", response.body());
                        return response;
                    }
                    throw new IllegalStateException("Analytics service returned status "
                            + response.statusCode() + ": " + response.body());
                })
                .thenApply(response -> (Void) null)
                .exceptionally(ex -> {
                    logger.error("❌ [AnalyticsService] ingest event failed: {}", ex.getMessage(), ex);
