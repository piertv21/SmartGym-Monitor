                        ...
                        msg -> {
                            try {
                                ...
                                MachineUsageMessage machineUsageMessage =
                                        json.mapTo(MachineUsageMessage.class);

                                System.out.println(
                                        "[MachineUsageVerticle] Machine usage event received: "
                                                + machineUsageMessage);

                                embeddedService
                                        .processMachineUsage(machineUsageMessage)
                                        .thenAccept(
                                                result ->
                                                        System.out.println(
                                                                "[MachineUsageVerticle] Machine usage event processed successfully"))
                                        .exceptionally(
                                                ex -> {
                                                    System.err.println(
                                                            "❌ Failed to process machine usage event: "
                                                                    + ex.getMessage());
                                                    return null;
                                                });
                                ...
                                System.err.println(
                                        "❌ Invalid machine usage event payload: " + e.getMessage());
                            }
                        });
    }
}
