...
public class VertxMqttClientAdapter extends AbstractVerticle {
    ...
    public void start() {
        final boolean useSsl =
                "tls".equalsIgnoreCase(MqttConfig.BROKER_PROTOCOL)
                        || "ssl".equalsIgnoreCase(MqttConfig.BROKER_PROTOCOL);

        final MqttClientOptions options =
                new MqttClientOptions().setSsl(useSsl).setTrustAll(useSsl);
                                    ...
                                    if (subResult.succeeded()) {
                                        ...
                                        System.err.println(
                                                "MQTT subscription failed: "
                                                        + subResult.cause().getMessage());
                                    }
                                });
                    } else {
                        System.err.println(
                                "MQTT connection failed: " + result.cause().getMessage());
                    }
                });
                    ...
                    try {
                        if (topic.equals(baseTopic + "/gym-access")) {
                            GymAccessMessage gymAccessMessage =
                                    mapper.readValue(payload, GymAccessMessage.class);
                            vertx.eventBus()
                                    .publish(
                        ...
                        if (topic.equals(baseTopic + "/area-access")) {
                            AreaAccessMessage areaAccessMessage =
                                    mapper.readValue(payload, AreaAccessMessage.class);
                            vertx.eventBus()
                                    .publish(
                        ...
                        if (topic.equals(baseTopic + "/machine-usage")) {
                            MachineUsageMessage machineUsageMessage =
                                    mapper.readValue(payload, MachineUsageMessage.class);
                            vertx.eventBus()
                                    .publish(
                        ...
                        if (topic.equals(baseTopic + "/device-status")) {
                            DeviceStatusMessage deviceStatusMessage =
                                    mapper.readValue(payload, DeviceStatusMessage.class);
                            JsonObject json = JsonObject.mapFrom(deviceStatusMessage);

                            if (deviceStatusMessage.getDeviceId() != null
                                    && !deviceStatusMessage.getDeviceId().isBlank()) {
                                sensorStatusMap.put(deviceStatusMessage.getDeviceId(), json);
                            }

                            vertx.eventBus()
                                    .publish(EventBusAddresses.DEVICE_STATUS, json.encode());
                            return;
                        }
                        ...
                        if (topic.startsWith(baseTopic + "/") && topic.endsWith("/status")) {
                            ...
                            if (statusMessage.getDeviceId() != null
                                    && !statusMessage.getDeviceId().isBlank()) {
                                sensorStatusMap.put(statusMessage.getDeviceId(), json);
                            }

                            System.out.println(
                                    "Updated status for device ["
                        ...
                        System.err.println(
                                "Error parsing MQTT message on topic ["
                                        + topic
                                        + "]: "
                                        + e.getMessage());
                    }
    ...
    public static void publish(String topic, String message) {
        if (clientInstance != null && clientInstance.isConnected()) {
            clientInstance.publish(
                    topic, Buffer.buffer(message), MqttQoS.AT_LEAST_ONCE, false, false);
