...
public class AreaServiceHttpAdapter implements AreaServicePort {
    ...
    public CompletableFuture<Void> processAreaAccess(AreaAccessMessage message) {
        ...
        try {
            ...
            return CompletableFuture.failedFuture(
                    new IllegalArgumentException("Invalid area access payload", ex));
        }
        ...
        return httpClient
                ...
                .thenApply(
                        response -> {
                            logger.info(
                                    "[AreaService] area access response received - Status: {}",
                                    response.statusCode());
                ...
                .exceptionally(
                        ex -> {
                            logger.error(
                                    " [AreaService] area access failed: {}", ex.getMessage(), ex);
    ...
    public CompletableFuture<Void> processAreaExit(AreaAccessMessage message) {
        ...
        try {
            ...
            return CompletableFuture.failedFuture(
                    new IllegalArgumentException("Invalid area exit payload", ex));
        }
        ...
        return httpClient
                ...
                .thenApply(
                        response -> {
                            logger.info(
                                    " [AreaService] area exit response received - Status: {}",
                                    response.statusCode());
                            if (response.statusCode() >= 200 && response.statusCode() < 300) {
                                logger.debug("   Response body: {}", response.body());
                                return response;
                ...
                .exceptionally(
                        ex -> {
                            logger.error(
                                    " [AreaService] area exit failed: {}", ex.getMessage(), ex);
                            throw new RuntimeException(ex);
                        });
    }
}
