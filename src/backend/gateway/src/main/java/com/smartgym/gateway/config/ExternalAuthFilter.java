...
public class ExternalAuthFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (path.startsWith("/actuator") || path.endsWith("/login") || path.endsWith("/register")) {
        ...
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            ...
            log.warn(
                    "[GATEWAY] BLOCKED {} {} from {} -> route={} reason=missing_bearer_token",
                    method,
                    path,
                    remoteIp,
                    routeId);
        ...
        try {
            ...
            exchange.getResponse().setStatusCode(HttpStatus.UNAUTHORIZED);
            log.warn(
                    "[GATEWAY] BLOCKED {} {} from {} -> route={} reason=invalid_jwt",
                    method,
                    path,
                    remoteIp,
                    routeId);
